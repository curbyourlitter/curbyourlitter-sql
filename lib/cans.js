...
function getCanColumnsData(config) {
    return ["'can' AS type", 'cartodb_id', 'location AS can_location', 'maintained_by', 'pick_up_schedule', 'type AS cantype'];
}

function getCanColumnsDetails(config) {
    return ["'can' AS type", 'cartodb_id', 'location AS can_location', 'maintained_by', 'pick_up_schedule', 'type AS cantype', 'ST_X(the_geom) AS longitude', 'ST_Y(the_geom) AS latitude'];
}

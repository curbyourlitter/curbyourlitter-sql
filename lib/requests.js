...
function getRequestColumnsDetails(config) {
    return ["'request' AS type", 'cartodb_id', 'can_type', 'can_subtype', 'comment', 'image', 'ST_X(the_geom) AS longitude', 'ST_Y(the_geom) AS latitude', 'name', '(added AT TIME ZONE \'' + config.timezone + '\')::text AS date'];
}
...
function getRequestColumnsMap(config) {
    return ['*', 'ST_X(the_geom) AS longitude', 'ST_Y(the_geom) AS latitude', '(added AT TIME ZONE \'' + config.timezone + '\')::text AS date'];
}

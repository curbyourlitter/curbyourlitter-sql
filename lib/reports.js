...
exports.getReportSqlDetails = getReportSqlDetails;
...
function getReportColumnsDetails(config) {
    return ["'report' AS type", 'r.complaint_type', 'r.cartodb_id', 'r.descriptor', 'r.incident_address', 'r.intersection_street1', 'r.intersection_street2', 'ST_X(r.the_geom) AS longitude', 'ST_Y(r.the_geom) AS latitude', '(r.created_date AT TIME ZONE \'' + config.timezone + '\')::text AS date', 'c.description'];
}
...
function getReportSqlDetails(id, config) {
